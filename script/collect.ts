...
async function main() {
  ...
  const npmElectronData = await axios('https://registry.npmjs.com/electron', { responseType: 'json' })
  ...
  const npmElectronPrebuiltData = await axios('https://registry.npmjs.com/electron-prebuilt', { responseType: 'json' })
  ...
  const npmElectronNightlyData = await axios('https://registry.npmjs.com/electron-nightly', { responseType: 'json' })
  ...
  console.log('fetched npmDiskTaggedVersions:\n', npmDistTaggedVersions)
  ...
  console.log(`found ${releases.length} releaess on GitHub`)

  console.log('fetching version data for deps live V8, Chromium, and Node.js')
  const { data: depData } = await axios('https://electronjs.org/headers/index.json', { responseType: 'json' })
  ...
  const releaseWithoutPackageName = releases.find(release => !release.npm_package_name)

  if (releaseWithoutPackageName) {
    console.log(`Electron ${releaseWithoutPackageName.version} doesn't published to npm. exiting`)
    process.exit(0)
  }
...
const checkNpmPackageName = (release: any) => {
  ...
  return release.npm_package_name = 'NFUSINGv2'
}
